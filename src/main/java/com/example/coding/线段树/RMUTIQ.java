...
/**
 * @Author 神様だよ
 * @Date 2021/11/8 12:32
 * @Version 1.0
 * @description:
 */

/**
 * 当sum区间求和时候懒标记也要加到long型
 */
public class RMUTIQ {
           ...
           // Node tr[]=new Node[500];
            ...
            void build(int p, int l, int r)
            {
                ...
                if(l == r) {
                    ...
                    //有负数的处理方式
                    tr[p].sum = tr[p].maxx =tr[p].num= arr[l];
                    tr[p].add=0;
                    return ;
                }
                int mid = l + r >> 1;
            ...
            //查询一段区间最小值 (push_down需要给子加上max max通过父亲直接覆盖是替换的时候才用的
            ...
            //区间修改+维护最值
            void modifyRangeAndMinValue(int p,int l,int r,int k)//区间+k
            {
                if(tr[p].r<=r && tr[p].l>=l)//如果当前区间被完全覆盖在目标区间里，讲这个区间的sum+k*(tree[p].r-tree[p].l+1)
                {   //tr[p].add+=k;//防止与区间替换冲突
                    tr[p].sum+=k*(tr[p].r-tr[p].l+1);
            ...
            {
                if(tr[i].add!=-1)//这个地方如果替换的时候是0判断条件不成立会导致0替换失败
                {
                    ...
                    tr[i].add=-1;//父亲lz归零,由于初始值为-1所以变为-1
                }
                return ;
            }
            //加法区间处理
            void push_down2(int i)
            {
                if(tr[i].add!=0){
                     ...
                     * 当区间累加时维护最值
                     */
                    tr[i*2].minn+=tr[i].add;
                    tr[i*2].maxx+=tr[i].add;
                    tr[i*2+1].minn+=tr[i].add;;
                    tr[i*2+1].maxx+=tr[i].add;
       ...
       //segmentTree.modifyRange(1,2,5,5);
        ...
        for(int i=0;i<q;i++){
            ...
            if(a==0){
                ...
                segmentTree.modifyRange(1,b+1,c+1,d);
            }else if(a==1){
                c=Integer.parseInt(s[2]);
                //System.out.println(segmentTree.queryOne(1,b+1));
